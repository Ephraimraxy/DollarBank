...
export default function AuthFlow({ currentView, setView, onLogin, darkMode, setDarkMode }: Props) {
  ...
  const renderSignIn = () => (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mt-12 space-y-4">
        <button onClick={handleLogin} className="w-full bg-red-600 text-white font-black uppercase tracking-[0.2em] py-5 rounded-[24px] shadow-2xl hover:bg-red-700 active:scale-95 transition-all text-xs">
          Open Secure Vault
        </button>
        <button onClick={() => setView(ViewState.SIGN_UP)} className={`w-full font-black uppercase tracking-widest py-5 rounded-[24px] border-2 text-xs transition-all ${
            darkMode ? 'border-gray-800 text-gray-400 hover:bg-gray-800' : 'border-gray-100 text-gray-500 hover:bg-gray-50'
        }`}>
          Create New Identity
        </button>
      </div>
  ...
  const renderSignUp = () => (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="space-y-8">
        <AuthInput icon={<User size={18} />} label="Legal Full Name" type="text" value="" onChange={() => {}} darkMode={darkMode} />
        <AuthInput icon={<Mail size={18} />} label="Secure Email" type="email" value="" onChange={() => {}} darkMode={darkMode} />
        <AuthInput icon={<Lock size={18} />} label="Vault Passphrase" type="password" value="" onChange={() => {}} darkMode={darkMode} />
      </div>

      <div className="mt-12 space-y-4">
        <button onClick={handleLogin} className="w-full bg-red-600 text-white font-black uppercase tracking-widest py-5 rounded-[24px] shadow-2xl hover:bg-red-700 active:scale-95 transition-all text-xs">
          Register Platinum Identity
        </button>
        <button onClick={() => setView(ViewState.SIGN_IN)} className={`w-full font-black uppercase tracking-widest py-5 rounded-[24px] border-2 text-xs transition-all ${
            darkMode ? 'border-gray-800 text-gray-400 hover:bg-gray-800' : 'border-gray-100 text-gray-500 hover:bg-gray-50'
        }`}>
  ...
  const renderReverify = () => (
    <div className="animate-in zoom-in-95 duration-700 flex flex-col items-center justify-center min-h-[70vh] text-center">
      ...
      <div className="w-full space-y-4 px-4">
        ...
        <button onClick={() => setView(ViewState.SUPPORT)} className={`w-full font-black uppercase tracking-widest py-5 rounded-[24px] border-2 text-xs transition-all ${
            darkMode ? 'border-gray-800 text-gray-400 hover:bg-gray-800' : 'border-gray-100 text-gray-500 hover:bg-gray-50'
        }`}>
          Contact Concierge
        </button>
  ...
  <div className="relative group">
    ...
    <div className="relative flex items-center">
      ...
      <input 
        ...
        className={`w-full bg-transparent border-b-2 py-3 pl-8 pr-10 outline-none font-bold text-sm transition-all ${
            darkMode ? 'border-gray-800 text-white focus:border-red-600' : 'border-gray-200 text-gray-900 focus:border-red-600'
        }`}
      />
      {showToggle && (
